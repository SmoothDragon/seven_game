...
- **Responsive**: at `max-width:900px`, `.main-wrapper` becomes `flex-direction: column` so sidebars stack below the playfield. At `min-width:1100px` the layout flips into a *wide* mode: `.main-wrapper`'s max-width grows to `1200px`, `.container`'s to `940px`, and `#game-play-area` becomes a 2-column CSS grid (`grid-template-columns: minmax(360px, 1fr) auto`) with named areas `header / board / colfeed / input / hint / msg` stacked in the left column and a single `keyboard` area that **spans all six left rows**. `#keyboard` sets `width:460px; margin:0; align-self:center` so the on-screen keyboard sits alongside the board, vertically centered against the entire left stack — this lets the whole game (board + keyboard) fit on a 13" MacBook (1440×900 effective) without vertical scrolling. Between 901px and 1099px viewport width, the layout stays the original stacked column-of-rows. Past 7 mode is unaffected because `#past7-section` is a sibling of `#game-play-area`, not a child of it.
...
**Initialization** (`initPast7()`): stops any prior replay, hides `#past7-replay`, shows `#past7-grid`, calls `updatePast7VariantButtons()`, builds `past7Days` by calling `puzzleForDateVariant(date, past7Variant)` for each date (tagging each day with `day.variant` and marking `loaded:false`), renders the grid with a "Loading…" state, and — if `db` is available — kicks off one `fetchBestScoreForDay(day)` per day in parallel. Each fetch queries the variant's collection filtered to `day.date`, applies the **zero-hint-only → fewest guesses → fastest time** ordering described in §6.4, and picks the best-ranked entry that has a non-empty `guess_log` (falling back to the top-ranked entry if none are replayable). Each completion flips `day.loaded = true` and re-renders the grid.
